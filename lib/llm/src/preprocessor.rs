...
pub struct OpenAIPreprocessor {
    ...
    /// unreadable.
    #[cfg(feature = "lightseek-mm")]
    lightseek_counter: Option<Arc<lightseek_mm::LightseekMmCounter>>,
    ...
    /// KV-cache block size advertised by the worker via MDC. Used to align
    /// `block_mm_infos` with the worker-side cache layout. `0` means MM
    /// routing is a no-op.
    #[cfg(feature = "lightseek-mm")]
...
impl OpenAIPreprocessor {
    ...
    pub fn new_with_parts(
        ...
        #[cfg(feature = "lightseek-mm")]
        let lightseek_inputs: Option<(String, std::path::PathBuf)> =
            mdc_model_dir(&mdc).map(|p| (mdc.source_path().to_string(), p));
        ...
        #[cfg(feature = "lightseek-mm")]
        let (lightseek_counter, image_token_id) = match lightseek_inputs {
            Some((model_id, model_dir)) => {
    ...
    pub async fn gather_multi_modal_data<R: OAIChatLikeRequest>(
        ...
        if !fetch_tasks.is_empty() {
            ...
            for ((type_str, content_part), result) in
                fetch_tasks.into_iter().zip(results.into_iter())
            {
                ...
                if type_str == "image_url" {
                    ...
                    if shape.len() >= 2 {
                        ...
                        let url_str = match content_part {
                            ChatCompletionRequestUserMessageContentPart::ImageUrl(p) => {
                        ...
                        if let Some(counter) = self.lightseek_counter.as_ref() {
    ...
    /// - `lightseek_counter` is unavailable,
    ...
    pub fn gather_mm_exact_routing_info(
        ...
        let Some(counter) = self.lightseek_counter.as_ref() else {
            tracing::debug!(
                target: "lightseek_mm",
                "lightseek_counter unavailable; skipping MM routing info"
            );
            return Ok(());
        };
